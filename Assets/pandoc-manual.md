...
- [Extensions](#extensions){#toc-extensions}
  ...
  - [Other extensions](#other-extensions){#toc-other-extensions}
    ...
    - [Extension: `smart_quotes`
      (org)](#extension-smart_quotes-org){#toc-extension-smart_quotes-org}
    - [Extension: `special_strings`
      (org)](#extension-special_strings-org){#toc-extension-special_strings-org}
    - [Extension:
...
- [Pandoc's Markdown](#pandocs-markdown){#toc-pandocs-markdown}
  ...
  - [Tables](#tables){#toc-tables}
    ...
    - [Extension:
      `table_attributes`](#extension-table_attributes){#toc-extension-table_attributes}
  ...
  - [Non-default
    ...
    - [Extension:
      `mmd_header_identifiers`](#extension-mmd_header_identifiers){#toc-extension-mmd_header_identifiers}
    - [Extension:
...
- [Slide shows](#slide-shows){#toc-slide-shows}
  ...
  - [Speaker notes](#speaker-notes){#toc-speaker-notes}
    - [Speaker notes on the title slide
      (PowerPoint)](#speaker-notes-on-the-title-slide-powerpoint){#toc-speaker-notes-on-the-title-slide-powerpoint}
...
goes to *stdout* by default. For output to a file, use the
`-o`/`--output` option:
...
[`multirow`](https://ctan.org/pkg/multirow) (if the document contains a
table with cells that cross multiple rows),
...
[`csquotes`](https://ctan.org/pkg/csquotes) package will be used for
[typography](#typography) if the `csquotes` variable or metadata field
is set to a true value. The [`natbib`](https://ctan.org/pkg/natbib),
[`biblatex`](https://ctan.org/pkg/biblatex),
[`bibtex`](https://ctan.org/pkg/bibtex), and
[`biber`](https://ctan.org/pkg/biber) packages can optionally be used
for [citation rendering](#citation-rendering). If math with `\cancel`,
`\bcancel`, or `\xcancel` is used, the
[`cancel`](https://ctan.org/pkg/cancel) package is needed. The following
packages will be used to improve output quality if present, but pandoc
does not require them to be present:
[`upquote`](https://ctan.org/pkg/upquote) (for straight quotes in
verbatim environments), [`microtype`](https://ctan.org/pkg/microtype)
(for better spacing adjustments),
[`parskip`](https://ctan.org/pkg/parskip) (for better inter-paragraph
spaces), [`xurl`](https://ctan.org/pkg/xurl) (for better line breaks in
URLs), and [`footnotehyper`](https://ctan.org/pkg/footnotehyper) or
...
:   Specify a set of default option settings. *FILE* is a YAML or JSON
    ...
    options for document conversion, including input and output files,
    can be set using a defaults file. The file will be searched for
    first in the working directory, and then in the `defaults`
    subdirectory of the user data directory (see `--data-dir`). The
    `.yaml` extension will be added if *FILE* lacs an extension. See the
    section [Defaults files](#defaults-files) for more information on
    the file format. Settings from the defaults file may be overridden
    or extended by subsequent options on the command line.
...
:   The method to use for code syntax highlighting. Setting a specific
    *STYLE* causes highlighting to be performed with the internal
    highlighting engine, using KDE syntax definitions and styles. The
    `idiomatic` method uses a format-specific highlighter if one is
    available, or the default style if the target format has no
    idiomatic highlighting method. Setting this option to `none`
    disables all syntax highlighting. The `default` method uses a
    format-specific default.

    The default for HTML, EPUB, Docx, Ms, Man, and LaTeX output is to
    use the internal highlighter with the default style; for Typst it is
    to use Typst's own syntax highlighting system.
...
:   Prints a JSON version of a highlighting style, which can be
    ...
    `--syntax-highlighting`. This option may be used with
    `-o`/`--output` to redirect output to a file, but `-o`/`--output`
...
the form of a YAML or JSON file. Examples in this section will be given
in YAML, but the equivalent forms in JSON will also work.
...
+:================================+:==================================+
|     foo.md                      | ``` {.sourceCode .yaml}           |
|                                 | input-file: foo.md                |
|                                 | ```                               |
+---------------------------------+-----------------------------------+
|     foo.md bar.md               | ``` {.sourceCode .yaml}           |
|                                 | input-files:                      |
|                                 |   - foo.md                        |
|                                 |   - bar.md                        |
|                                 | ```                               |
...
| command line                       | defaults file                     |
+:===================================+:==================================+
|     --from markdown+emoji          | ``` {.sourceCode .yaml}           |
|                                    | from: markdown+emoji              |
|                                    | ```                               |
|                                    |                                   |
|                                    | ``` {.sourceCode .yaml}           |
...
|                                    | ```                               |
+------------------------------------+-----------------------------------+
|     --output foo.pdf               | ``` {.sourceCode .yaml}           |
|                                    | output-file: foo.pdf              |
|                                    | ```                               |
+------------------------------------+-----------------------------------+
|     --output -                     | ``` {.sourceCode .yaml}           |
|                                    | output-file:                      |
|                                    | ```                               |
+------------------------------------+-----------------------------------+
|     --data-dir dir                 | ``` {.sourceCode .yaml}           |
|                                    | data-dir: dir                     |
|                                    | ```                               |
+------------------------------------+-----------------------------------+
|     --defaults file                | ``` {.sourceCode .yaml}           |
|                                    | defaults:                         |
|                                    | - file                            |
|                                    | ```                               |
+------------------------------------+-----------------------------------+
|     --verbose                      | ``` {.sourceCode .yaml}           |
|                                    | verbosity: INFO                   |
|                                    | ```                               |
+------------------------------------+-----------------------------------+
|     --quiet                        | ``` {.sourceCode .yaml}           |
|                                    | verbosity: ERROR                  |
|                                    | ```                               |
+------------------------------------+-----------------------------------+
|     --fail-if-warnings             | ``` {.sourceCode .yaml}           |
|                                    | fail-if-warnings: true            |
|                                    | ```                               |
+------------------------------------+-----------------------------------+
|     --sandbox                      | ``` {.sourceCode .yaml}           |
|                                    | sandbox: true                     |
|                                    | ```                               |
+------------------------------------+-----------------------------------+
|     --log=FILE                     | ``` {.sourceCode .yaml}           |
|                                    | log-file: FILE                    |
|                                    | ```                               |
+------------------------------------+-----------------------------------+
...
| command line                         | defaults file                     |
+:=====================================+:==================================+
|     --shift-heading-level-by -1      | ``` {.sourceCode .yaml}           |
...
|                                      | ```                               |
+--------------------------------------+-----------------------------------+
|     --file-scope                     | ``` {.sourceCode .yaml}           |
|                                      | file-scope: true                  |
|                                      | ```                               |
+--------------------------------------+-----------------------------------+
|     --citeproc \                     | ``` {.sourceCode .yaml}           |
...
|                                      | ```                               |
+--------------------------------------+-----------------------------------+
|     --metadata key=value \           | ``` {.sourceCode .yaml}           |
|      --metadata key2                 | metadata:                         |
|                                      |   key: value                      |
|                                      |   key2: true                      |
|                                      | ```                               |
+--------------------------------------+-----------------------------------+
|     --metadata-file meta.yaml        | ``` {.sourceCode .yaml}           |
|                                      | metadata-files:                   |
|                                      |   - meta.yaml                     |
|                                      | ```                               |
|                                      |                                   |
...
|                                      | ```                               |
+--------------------------------------+-----------------------------------+
|     --preserve-tabs                  | ``` {.sourceCode .yaml}           |
|                                      | preserve-tabs: true               |
|                                      | ```                               |
+--------------------------------------+-----------------------------------+
|     --tab-stop 8                     | ``` {.sourceCode .yaml}           |
|                                      | tab-stop: 8                       |
|                                      | ```                               |
+--------------------------------------+-----------------------------------+
|     --track-changes accept           | ``` {.sourceCode .yaml}           |
|                                      | track-changes: accept             |
|                                      | ```                               |
+--------------------------------------+-----------------------------------+
|     --extract-media dir              | ``` {.sourceCode .yaml}           |
|                                      | extract-media: dir                |
|                                      | ```                               |
+--------------------------------------+-----------------------------------+
|     --abbreviations abbrevs.txt      | ``` {.sourceCode .yaml}           |
|                                      | abbreviations: abbrevs.txt        |
|                                      | ```                               |
+--------------------------------------+-----------------------------------+
|     --trace                          | ``` {.sourceCode .yaml}           |
|                                      | trace: true                       |
|                                      | ```                               |
+--------------------------------------+-----------------------------------+
...
| command line                       | defaults file                     |
+:===================================+:==================================+
|     --standalone                   | ``` {.sourceCode .yaml}           |
|                                    | standalone: true                  |
|                                    | ```                               |
+------------------------------------+-----------------------------------+
|     --template letter              | ``` {.sourceCode .yaml}           |
|                                    | template: letter                  |
|                                    | ```                               |
+------------------------------------+-----------------------------------+
|     --variable key=val \           | ``` {.sourceCode .yaml}           |
|       --variable key2              | variables:                        |
|                                    |   key: val                        |
|                                    |   key2: true                      |
|                                    | ```                               |
+------------------------------------+-----------------------------------+
|     --eol nl                       | ``` {.sourceCode .yaml}           |
|                                    | eol: nl                           |
|                                    | ```                               |
+------------------------------------+-----------------------------------+
|     --dpi 300                      | ``` {.sourceCode .yaml}           |
|                                    | dpi: 300                          |
|                                    | ```                               |
+------------------------------------+-----------------------------------+
|     --wrap preserve                | ``` {.sourceCode .yaml}           |
|                                    | wrap: "preserve"                  |
|                                    | ```                               |
+------------------------------------+-----------------------------------+
|     --columns 72                   | ``` {.sourceCode .yaml}           |
|                                    | columns: 72                       |
|                                    | ```                               |
+------------------------------------+-----------------------------------+
|     --table-of-contents            | ``` {.sourceCode .yaml}           |
...
|                                    | ```                               |
+------------------------------------+-----------------------------------+
|     --toc-depth 3                  | ``` {.sourceCode .yaml}           |
|                                    | toc-depth: 3                      |
|                                    | ```                               |
+------------------------------------+-----------------------------------+
|     --strip-comments               | ``` {.sourceCode .yaml}           |
|                                    | strip-comments: true              |
|                                    | ```                               |
+------------------------------------+-----------------------------------+
|     --no-highlight                 | ``` {.sourceCode .yaml}           |
|                                    | syntax-highlighting: 'none'       |
|                                    | ```                               |
+------------------------------------+-----------------------------------+
|     --syntax-highlighting kate     | ``` {.sourceCode .yaml}           |
|                                    | syntax-highlighting: kate         |
|                                    | ```                               |
+------------------------------------+-----------------------------------+
|     --syntax-definition mylang.xml | ``` {.sourceCode .yaml}           |
|                                    | syntax-definitions:               |
|                                    |   - mylang.xml                    |
|                                    | ```                               |
|                                    |                                   |
...
|                                    | ```                               |
+------------------------------------+-----------------------------------+
|     --include-in-header inc.tex    | ``` {.sourceCode .yaml}           |
|                                    | include-in-header:                |
|                                    |   - inc.tex                       |
|                                    | ```                               |
+------------------------------------+-----------------------------------+
|     --include-before-body inc.tex  | ``` {.sourceCode .yaml}           |
|                                    | include-before-body:              |
|                                    |   - inc.tex                       |
|                                    | ```                               |
+------------------------------------+-----------------------------------+
|     --include-after-body inc.tex   | ``` {.sourceCode .yaml}           |
|                                    | include-after-body:               |
|                                    |   - inc.tex                       |
|                                    | ```                               |
+------------------------------------+-----------------------------------+
|     --resource-path .:foo          | ``` {.sourceCode .yaml}           |
|                                    | resource-path: ['.','foo']        |
|                                    | ```                               |
+------------------------------------+-----------------------------------+
|     --request-header foo:bar       | ``` {.sourceCode .yaml}           |
|                                    | request-headers:                  |
|                                    |   - ["User-Agent", "Mozilla/5.0"] |
|                                    | ```                               |
+------------------------------------+-----------------------------------+
|     --no-check-certificate         | ``` {.sourceCode .yaml}           |
...
| command line                         | defaults file                     |
+:=====================================+:==================================+
|     --self-contained                 | ``` {.sourceCode .yaml}           |
|                                      | self-contained: true              |
|                                      | ```                               |
+--------------------------------------+-----------------------------------+
|     --link-images                    | ``` {.sourceCode .yaml}           |
|                                      | link-images: true                 |
|                                      | ```                               |
+--------------------------------------+-----------------------------------+
|     --html-q-tags                    | ``` {.sourceCode .yaml}           |
|                                      | html-q-tags: true                 |
|                                      | ```                               |
+--------------------------------------+-----------------------------------+
|     --ascii                          | ``` {.sourceCode .yaml}           |
|                                      | ascii: true                       |
|                                      | ```                               |
+--------------------------------------+-----------------------------------+
|     --reference-links                | ``` {.sourceCode .yaml}           |
|                                      | reference-links: true             |
|                                      | ```                               |
+--------------------------------------+-----------------------------------+
|     --reference-location block       | ``` {.sourceCode .yaml}           |
|                                      | reference-location: block         |
|                                      | ```                               |
+--------------------------------------+-----------------------------------+
|     --figure-caption-position=above  | ``` {.sourceCode .yaml}           |
|                                      | figure-caption-position: above    |
|                                      | ```                               |
+--------------------------------------+-----------------------------------+
|     --table-caption-position=below   | ``` {.sourceCode .yaml}           |
|                                      | table-caption-position: below     |
|                                      | ```                               |
+--------------------------------------+-----------------------------------+
|     --markdown-headings atx          | ``` {.sourceCode .yaml}           |
|                                      | markdown-headings: atx            |
|                                      | ```                               |
+--------------------------------------+-----------------------------------+
|     --list-tables                    | ``` {.sourceCode .yaml}           |
|                                      | list-tables: true                 |
|                                      | ```                               |
+--------------------------------------+-----------------------------------+
|     --top-level-division chapter     | ``` {.sourceCode .yaml}           |
|                                      | top-level-division: chapter       |
|                                      | ```                               |
+--------------------------------------+-----------------------------------+
|     --number-sections                | ``` {.sourceCode .yaml}           |
|                                      | number-sections: true             |
|                                      | ```                               |
+--------------------------------------+-----------------------------------+
|     --number-offset=1,4              | ``` {.sourceCode .yaml}           |
|                                      | number-offset: \[1,4\]            |
|                                      | ```                               |
+--------------------------------------+-----------------------------------+
|     --listings                       | ``` {.sourceCode .yaml}           |
|                                      | listings: true                    |
|                                      | ```                               |
+--------------------------------------+-----------------------------------+
|     --list-of-figures                | ``` {.sourceCode .yaml}           |
...
|                                      | ```                               |
+--------------------------------------+-----------------------------------+
|     --list-of-tables                 | ``` {.sourceCode .yaml}           |
|                                      | list-of-tables: true              |
|                                      | ```                               |
+--------------------------------------+-----------------------------------+
|     --lot                            | ``` {.sourceCode .yaml}           |
...
|                                      | ```                               |
+--------------------------------------+-----------------------------------+
|     --incremental                    | ``` {.sourceCode .yaml}           |
|                                      | incremental: true                 |
|                                      | ```                               |
+--------------------------------------+-----------------------------------+
|     --slide-level 2                  | ``` {.sourceCode .yaml}           |
|                                      | slide-level: 2                    |
|                                      | ```                               |
+--------------------------------------+-----------------------------------+
|     --section-divs                   | ``` {.sourceCode .yaml}           |
|                                      | section-divs: true                |
|                                      | ```                               |
+--------------------------------------+-----------------------------------+
|     --email-obfuscation references   | ``` {.sourceCode .yaml}           |
|                                      | email-obfuscation: references     |
|                                      | ```                               |
+--------------------------------------+-----------------------------------+
|     --id-prefix ch1                  | ``` {.sourceCode .yaml}           |
|                                      | identifier-prefix: ch1            |
|                                      | ```                               |
+--------------------------------------+-----------------------------------+
|     --title-prefix MySite            | ``` {.sourceCode .yaml}           |
|                                      | title-prefix: MySite              |
|                                      | ```                               |
+--------------------------------------+-----------------------------------+
|     --css styles/screen.css  \       | ``` {.sourceCode .yaml}           |
|       --css styles/special.css       | css:                              |
|                                      |   - styles/screen.css             |
|                                      |   - styles/special.css            |
|                                      | ```                               |
+--------------------------------------+-----------------------------------+
|     --reference-doc my.docx          | ``` {.sourceCode .yaml}           |
|                                      | reference-doc: my.docx            |
|                                      | ```                               |
+--------------------------------------+-----------------------------------+
|     --epub-cover-image cover.jpg     | ``` {.sourceCode .yaml}           |
|                                      | epub-cover-image: cover.jpg       |
|                                      | ```                               |
+--------------------------------------+-----------------------------------+
|     --epub-title-page=false          | ``` {.sourceCode .yaml}           |
|                                      | epub-title-page: false            |
|                                      | ```                               |
+--------------------------------------+-----------------------------------+
|     --epub-metadata meta.xml         | ``` {.sourceCode .yaml}           |
|                                      | epub-metadata: meta.xml           |
|                                      | ```                               |
+--------------------------------------+-----------------------------------+
|     --epub-embed-font special.otf \  | ``` {.sourceCode .yaml}           |
|       --epub-embed-font headline.otf | epub-fonts:                       |
|                                      |   - special.otf                   |
|                                      |   - headline.otf                  |
|                                      | ```                               |
+--------------------------------------+-----------------------------------+
|     --split-level 2                  | ``` {.sourceCode .yaml}           |
|                                      | split-level: 2                    |
|                                      | ```                               |
+--------------------------------------+-----------------------------------+
|     --chunk-template="%i.html"       | ``` {.sourceCode .yaml}           |
|                                      | chunk-template: "%i.html"         |
|                                      | ```                               |
+--------------------------------------+-----------------------------------+
|     --epub-subdirectory=""           | ``` {.sourceCode .yaml}           |
|                                      | epub-subdirectory: ''             |
|                                      | ```                               |
+--------------------------------------+-----------------------------------+
|     --ipynb-output best              | ``` {.sourceCode .yaml}           |
|                                      | ipynb-output: best                |
|                                      | ```                               |
+--------------------------------------+-----------------------------------+
|     --pdf-engine xelatex             | ``` {.sourceCode .yaml}           |
|                                      | pdf-engine: xelatex               |
|                                      | ```                               |
+--------------------------------------+-----------------------------------+
|     --pdf-engine-opt=--shell-escape  | ``` {.sourceCode .yaml}           |
|                                      | pdf-engine-opts:                  |
|                                      |   - '-shell-escape'               |
|                                      | ```                               |
|                                      |                                   |
...
|                                      | ```                               |
+--------------------------------------+-----------------------------------+
|     --bibliography logic.bib         | ``` {.sourceCode .yaml}           |
|                                      | bibliography: logic.bib           |
|                                      | ```                               |
+--------------------------------------+-----------------------------------+
|     --csl ieee.csl                   | ``` {.sourceCode .yaml}           |
|                                      | csl: ieee.csl                     |
|                                      | ```                               |
+--------------------------------------+-----------------------------------+
|     --citation-abbreviations ab.json | ``` {.sourceCode .yaml}           |
|                                      | citation-abbreviations: ab.json   |
|                                      | ```                               |
+--------------------------------------+-----------------------------------+
|     --natbib                         | ``` {.sourceCode .yaml}           |
|                                      | cite-method: natbib               |
|                                      | ```                               |
+--------------------------------------+-----------------------------------+
|     --biblatex                       | ``` {.sourceCode .yaml}           |
...
| command line                    | defaults file                     |
+:================================+:==================================+
|     --mathjax                   | ``` {.sourceCode .yaml}           |
|                                 | html-math-method:                 |
|                                 |   method: mathjax                 |
|                                 | ```                               |
+---------------------------------+-----------------------------------+
|     --mathml                    | ``` {.sourceCode .yaml}           |
|                                 | html-math-method:                 |
|                                 |   method: mathml                  |
|                                 | ```                               |
+---------------------------------+-----------------------------------+
|     --webtex                    | ``` {.sourceCode .yaml}           |
|                                 | html-math-method:                 |
|                                 |   method: webtex                  |
|                                 | ```                               |
+---------------------------------+-----------------------------------+
|     --katex                     | ``` {.sourceCode .yaml}           |
|                                 | html-math-method:                 |
|                                 |   method: katex                   |
|                                 | ```                               |
+---------------------------------+-----------------------------------+
|     --gladtex                   | ``` {.sourceCode .yaml}           |
|                                 | html-math-method:                 |
|                                 |   method: gladtex                 |
|                                 | ```                               |
+---------------------------------+-----------------------------------+
...
| command line                    | defaults file                     |
+:================================+:==================================+
|     --dump-args                 | ``` {.sourceCode .yaml}           |
|                                 | dump-args: true                   |
|                                 | ```                               |
+---------------------------------+-----------------------------------+
|     --ignore-args               | ``` {.sourceCode .yaml}           |
|                                 | ignore-args: true                 |
|                                 | ```                               |
+---------------------------------+-----------------------------------+
...
:   identifies the main language of the document using IETF language
    ...
    [`polyglossia`](https://ctan.org/pkg/polyglossia)) or ConTeXt. Use
    native pandoc [Divs and Spans](#divs-and-spans) with the `lang`
...
:   the base script direction, either `rtl` (right-to-left) or `ltr`
    (left-to-right). For bidirectional documents, native pandoc `span`s
    and `div`s with the `dir` attribute (value `rtl` or `ltr`) can be
    used to override the base direction in some output formats. This may
    not always be necessary if the final renderer (e.g. the browser,
...
:   Enables inclusion of most of the
    ...
    you use `--css`, this variable is set to `true` by default. You can
...
:   when using `--katex`, you can render display math equations flush
    left using [YAML metadata](#layout) or with `-M classoption=fleqn`.
...
:   highlight.js theme for code highlighting when using
    ...
    `monokai`). See the [highlight.js demo
    page](https://highlightjs.org/demo) for available themes.
...
:   font families for use with `xelatex` or `lualatex`: take the name of
    ...
    uses the [`xecjk`](https://ctan.org/pkg/xecjk) package if `xelatex`
    is used, or the [`luatexja`](https://ctan.org/pkg/luatexja) package
    if `lualatex` is used.
...
:   the PDF trailer ID; must be two PDF byte strings if set,
    ...
    See the section on [reproducible builds](#reproducible-builds).
...
:   text to be placed in running header or footer (see [ConTeXt Headers
    and
    Footers](https://wiki.contextgarden.net/Document_layout_and_layers/Headers_and_footers));
    repeat up to four times for different placement

`indenting`
:   controls indentation of paragraphs, e.g. `yes,small,next` (see
    [ConTeXt
    Indentation](https://wiki.contextgarden.net/Text_blocks/Typography/Indentation));
...
:   options for page margins and text arrangement (see [ConTeXt
    Layout](https://wiki.contextgarden.net/Document_layout_and_layers/Tutorials));
    repeat for multiple options
...
:   font families: take the name of any system font (see [ConTeXt Font
    Switching](https://wiki.contextgarden.net/Characters_words_and_fonts/Tutorials))
...
:   paper size, e.g. `letter`, `A4`, `landscape` (see [ConTeXt Paper
    Setup](https://wiki.contextgarden.net/Document_layout_and_layers/Paper_setup));
    repeat for multiple options
...
:   adds to the preamble the setup necessary to generate PDF/A of the
    ...
    PDFA](https://wiki.contextgarden.net/Input_and_compilation/PDF/PDFA)
    for more details.
...
:   when used in conjunction with `pdfa`, specifies the ICC profile to
    ...
    Profiles](https://wiki.contextgarden.net/Input_and_compilation/PDF/PDFX#ICC_profiles).
...
you can use an explicit attribute (assuming the `link_attributes`
extension is set):
...
:   The `lang` field will affect how the style is localized, for example
    ...
    used instead of `lang`, but this use is deprecated.) A BCP 47
    language tag is expected: for example, `en`, `de`, `en-US`, `fr-CA`,
    `ug-Cyrl`. The unicode extension syntax (after `-u-`) may be used to
    specify options for collation (sorting) more precisely. Here are
    some examples:
...
:   Either a string value or an object with fields `text` and `scheme`.
    ...
    `Legal deposit number`, `URN`, `OCLC number`,
...
:   Either a string value, or an object with fields `text`, `authority`,
    ...
    value](https://idpf.github.io/epub-registries/authorities/)
    (currently `AAT`, `BIC`, `BISAC`, `CLC`, `DDC`, `CLIL`, `EuroVoc`,
    `MEDTOP`, `LCSH`, `NDC`, `Thema`, `UDC`, and `WGS`) or an absolute
    IRI identifying a custom scheme. Valid values for `term` are defined
    by the scheme.
...
(assumes `--standalone`, see [Templates](#templates)):
...
HTML, EPUB, Docx, Ms, Man, Typst, and LaTeX/PDF output. To see a list of
...
- In reveal.js, it causes reveal.js's highlighting plugin to be used for
  ...
  `highlightjs-theme` variable.

- In Typst, it causes Typst's built-in highlighting to be used. (This is
  also the default for Typst.)

- In LaTeX, it causes the [`listings`](https://ctan.org/pkg/listings)
  package to be used. Note that `listings` does not support multi-byte
  encoding for source code. To handle UTF-8 you would need to use a
  custom template. This issue is fully documented here: [Encoding issue
  with the listings
  package](https://en.wikibooks.org/wiki/LaTeX/Source_Code_Listings#Encoding_issue).
...
is optimized for tagging. For example:
...
5.4. All `pandoc.*` packages, as well as the packages `re` and `lpeg`,
...
[^4]: Note that if `--file-scope` is used, a div written this way will
    be given an identifier of the form `FILE__refs`, to avoid duplicate
